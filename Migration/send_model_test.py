...
class Generator(BehaviorModelExecutor):
    ...
    def output(self):
        ...
        msg.insert((self.migration_cnt, self.my_var))
        print(f"Process ID:{os.getpid()}, Migration Count:{self.migration_cnt}")
        print(f"[{self.get_name()}][{self.my_var}]: {datetime.datetime.now()}")
        return msg
